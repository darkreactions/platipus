...
def get_concs(all_ingredients_df, chemdf_dict, ss_reagent_vols):
    ...
    summed_mmol = summed_mmol.rename(columns=replacement_cols)
    summed_molarity = summed_molarity.rename(columns=replacement_cols_mmol)

    summed_molarity.to_csv('./extra_data/stateset_molarities.csv', index_label='name')
    summed_mmol.to_csv('./extra_data/stateset_mmol.csv', index_label='name')
...
def get_stateset(stateset_mmol, stateset_molarities):
    ...
    combined.to_csv('./extra_data/final_data.csv', index_label='name')
    combined = pd.read_csv('./extra_data/final_data.csv', index_col='name')
...
def dump_model_pkl(model, model_name, set_num, iteration):
    pickle.dump(model, Path(f'./{model_name}/{model_name}_set{set_num}_it{iteration}_{datetime.now().strftime("%Y%m%d-%H%M%S")}.pkl').open('wb'))