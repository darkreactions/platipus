...
from utils import (read_pickle, find_avg_metrics, find_success_rate,
                   find_bcr, find_winning_models)
import matplotlib
matplotlib.use('Agg')


def plot_categorical_graph(stats_dict, dst, style_combinations, amine=None,
                           amine_index=None, models=None, show=False):
    ...
    for model in models:
        ...
        if amine and len(stats_dict[model]['accuracies']) > amine_index:
            ...
            num_examples = [i for i in range(
                len(stats_dict[model]['accuracies']))]
            if len(num_examples) != 1:
                # Plot line graphs for models with active learning
    ...
    if with_AL:
        ...
        prec.tick_params(axis='x', which='both', bottom=False,
                         top=False, labelbottom=False)
        rec.tick_params(axis='x', which='both', bottom=False,
                        top=False, labelbottom=False)
        bcr.tick_params(axis='x', which='both', bottom=False,
                        top=False, labelbottom=False)
    ...
    fig.legend(handles, labels, loc="lower center", ncol=num_cols,
               fontsize=16, bbox_to_anchor=[0.5, 0.04])

    # Move the subplots up
    plt.subplots_adjust(top=.95, bottom=.12)

    # Put x-axis label at the bottom
    fig.text(0.5, 0.02, "Number of samples given",
...
def plot_all_lines(stats_dict, dst, style_combinations, show=False):
    ...
    for model in models:
        ...
        if len(num_examples) != 1:
            # Plot line graphs for models with active learning
            acc.plot(num_examples, stats_dict[model]['accuracies'],
                     marker=linestyle, markersize=12, color=color,
                     linewidth=4, label=model)
            prec.plot(num_examples, stats_dict[model]['precisions'],
                      marker=linestyle, markersize=12, color=color,
                      linewidth=4, label=model)
            rec.plot(num_examples, stats_dict[model]['recalls'],
                     marker=linestyle, markersize=12, color=color,
                     linewidth=4, label=model)
            bcr.plot(num_examples, stats_dict[model]['bcrs'],
                     marker=linestyle, markersize=12, color=color,
                     linewidth=4, label=model)
        else:
            # Plot horizontal lines for models without active learning
            acc.axhline(y=stats_dict[model]['accuracies'],
                        linestyle=linestyle, linewidth=2, color=color, label=model)
            prec.axhline(y=stats_dict[model]['precisions'],
                         linestyle=linestyle, linewidth=2, color=color, label=model)
            rec.axhline(y=stats_dict[model]['recalls'],
                        linestyle=linestyle, linewidth=2, color=color, label=model)
            bcr.axhline(y=stats_dict[model]['bcrs'], linestyle=linestyle,
    ...
    if models:
        ...
        handles, labels = acc.get_legend_handles_labels()
        num_cols = int(
            len(labels) / 5) if len(labels) > 7 else int(len(labels)/2)
        fig.legend(handles, labels, loc="lower center", ncol=num_cols,
                   fontsize='x-large', bbox_to_anchor=[0.5, 0.03])
...
def plot_bcr_vs_success_rate(models, cv_stats, dst, names, success_volume, success_percentage, category=None,
    ...
    for model in models:
        if category:
            ...
            vol.scatter(success_volume, wanted_bcrs,
                        s=200, label=model, alpha=.6)
            per.scatter(success_percentage, wanted_bcrs,
                        s=200, label=model, alpha=.6)
        else:
            color = style_combinations[model]['color']
            marker = style_combinations[model]['marker']
            wanted_bcrs = find_bcr(model, cv_stats, names)
            vol.scatter(success_volume, wanted_bcrs, s=200,
                        label=model, alpha=.6, color=color, marker=marker)
            per.scatter(success_percentage, wanted_bcrs, s=200,
                        label=model, alpha=.6, color=color, marker=marker)
    ...
    fig.legend(handles, labels, loc="lower center",
               ncol=num_cols, fontsize='x-large')
...
def generate_style_combos(models):
    ...
    list_of_colors = ['violet', 'orangered', 'darkorange',
                      'seagreen', 'dodgerblue', 'darkviolet', 'r', 'violet']
...
def plot_all_graphs(cv_stats):
    ...
    cat_4 = [
        model for model in models_to_plot if 'amine' in model and not ('AL' in model)]
    cat_5 = [model for model in models_to_plot if 'AL' in model or (
        'PLATIPUS' in model)]
    ...
    hist_only = [
        model for model in models_to_plot if 'historical_only' in model]
    hist_amine = [
        model for model in models_to_plot if 'historical_amine' in model and not ('AL' in model)]
    amine_only = [
        model for model in models_to_plot if 'amine_only' in model and not ('AL' in model)]
    hist_amine_al = [model for model in models_to_plot if 'historical_amine_AL' in model or (
        'PLATIPUS' in model)]
    amine_only_al = [
        model for model in models_to_plot if 'amine_only_AL' in model]
    ...
    for cat in all_cats:
        ...
        for i, amine in enumerate(amines):
            graph_dst = '{0:s}/cv_metrics_{1:s}.png'.format(
                avg_graph_folder, amine)
            plot_categorical_graph(
                cv_stats, graph_dst, style_combinations, amine=amine, amine_index=i, models=models)

        # Plotting avg graphs for all models in one category
        avg_graph_dst = '{0:s}/average_metrics_{1:s}.png'.format(
            avg_graph_folder, cat)
        plot_categorical_graph(avg_stats, avg_graph_dst,
                               style_combinations, models=models)

        bcr_graph_dst = '{0:s}/bcr_against_success_{1:s}.png'.format(
            bcr_graph_folder, cat)
        plot_bcr_vs_success_rate(models, cv_stats, bcr_graph_dst,
                                 names, success_volume, success_percentage, category=cat)
    ...
    for subcat in list(all_subcats.keys()):
        ...
        plot_categorical_graph(avg_stats_cat, dst_path,
                               style_combinations, models=all_subcats[subcat])
    ...
    plot_all_lines(winning_stats, './results/winning_models.png',
                   style_combinations)

    # Plot graphs for all models
    plot_all_lines(
        avg_stats, './results/avg_metrics_all_models.png', style_combinations)
